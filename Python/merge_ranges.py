...
def merge_ranges(meetings):
    # always n log n time becuase of sorting. would be O(n) otherwise
    ...
    for current_start, current_end in sorted_meetings[1:]:
        ...
        if (current_start <= last_merged_end):
            merged_meetings[-1] = (last_merged_start, max(last_merged_end, current_end))
        else:
...
class Test(unittest.TestCase):
    ...
    def test_from_clues(self):
        actual = merge_ranges([(1, 2), (2, 3)])
        expected = [(1,3)]
        self.assertEqual(actual, expected)

    def test_another_clue(self):
